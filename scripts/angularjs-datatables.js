  ...
  .directive('ngRows', function ($compile) {
    return {
      ...
      controller: function ($scope) {
        ...
        //export result
        vm.tableToExcel = (function() {

            const workSheet = XLSX.utils.json_to_sheet(vm.filteredRows); //create worksheet using the table
              for (var i in workSheet) {
                if (typeof workSheet[i] != 'object') continue;
                let cell = XLSX.utils.decode_cell(i);
                workSheet[i].s = { //loop all the rows and column and change the font to arial
                  font: {
                    name: "Arial",
                  }
                }
                if (cell.r == 0) { // condition which only return true if the row is at "A1" (0 is the index of the A1 row)
                    workSheet[i].s = { // change the style of row 1 (A1,B1,C1,D1 etc.)
                      font: { 
                      ...
                      fill: {
                        fgColor: {rgb: "040788"} //changing the foreground color of row 1
                      }
                  };
                }
        }
      ...
      compile: function (row) {
        ...
        return function (vm, parent) {
          ...
          var exportToExcel =
            '<button class="btn btn-default" ng-click="tableToExcel()">' + "Export" + '</button>';
         

         var topControls = 
        //  selectAll + deselectAll +
            '<br><div class="row"><div class="col-sm-12 col-md-6"><label>' +
            exportToExcel +
            '</label></div><div class="col-sm-12 col-md-6"><div class="dataTables_filter"><label>Search:' +
          ...
          var bottomControls =
            '<div class="row"><div class="col-sm-12 col-md-5"><div class="dataTables_length"><label>Show ' +
            dropdown +
            ' entries</div></div>' +
            '<div class="col-sm-12 col-md-7"><div class="dataTables_paginate paging_simple_numbers"><ul class="pagination" >' +
            pagination +
            '<br><br>'+ pageInfo +'</ul></div></div></div></div>';
